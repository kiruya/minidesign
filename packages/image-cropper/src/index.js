...
Component({
  ...
  data: {
    ...
    watch: {
      ...
      canvas_top(value, that) {
        if (
          that.data.canvas_top < -that.data.height ||
          that.data.canvas_top > that.data.info.windowHeight
        ) {
      ...
      canvas_left(value, that) {
        if (
          that.data.canvas_left < -that.data.width ||
          that.data.canvas_left > that.data.info.windowWidth
        ) {
          that.data.canvas_overflow = true;
        } else {
          that.data.canvas_overflow = false;
      ...
      cut_top(value, that) {
        ...
        if (
          that.data.cut_top >
          that.data.info.windowHeight - that.data.height
        ) {
          that.setData({
  ...
  attached() {
    ...
    if (this.data.imgWidth && this.data.imgWidth.indexOf('%') != -1) {
      const width = this.data.imgWidth.replace('%', '');
      this.setData({
  ...
  methods: {
    ...
    setTransform(transform) {
      ...
      this.setData({
        ...
        imgLeft: transform.x
          ? this.data.imgLeft + transform.x
          : this.data.imgLeft,
        rotate: transform.rotate
          ? this.data.rotate + transform.rotate
          : this.data.rotate,
        scale
      });
    ...
    pushImg(src) {
      ...
      wx.getImageInfo({
        ...
        success: res => {
          ...
          if (this.data.init_imgWidth && this.data.init_imgHeight) {
    ...
    _move(event) {
      ...
      if (event.touches.length == 1) {
        // 单指拖动
        const left =
          event.touches[0].clientX - this.data.touch_img_relative[0].x;

        let top = event.touches[0].clientY - this.data.touch_img_relative[0].y;
        // left = left < 0 ? 0 : left;
        // top = top < 0 ? 0 : top;
        this.setData({
          imgLeft: left,
        ...
        let scale = this.data.scale * (hypotenuse / this.data.first_Hypotenuse);
        ...
        if (!this.data.disable_rotate) {
          const first_atan =
            (180 / Math.PI) *
            Math.atan2(touch_img_relative[0].y, touch_img_relative[0].x);
          let first_atan_old =
            (180 / Math.PI) *
            Math.atan2(
              this.data.touch_img_relative[0].y,
              this.data.touch_img_relative[0].x
            );
          let second_atan =
            (180 / Math.PI) *
            Math.atan2(touch_img_relative[1].y, touch_img_relative[1].x);
          let second_atan_old =
            (180 / Math.PI) *
            Math.atan2(
              this.data.touch_img_relative[1].y,
              this.data.touch_img_relative[1].x
            );
          //当前旋转的角度

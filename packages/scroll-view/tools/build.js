...
function copy(copyFileList) {
  ...
  return gulp.src(copyFileList, {cwd: srcPath, base: srcPath}).pipe(_.logger()).pipe(gulp.dest(distPath))
}
...
class BuildTask {
  ...
  init() {
      ...
      gulp.series(
        ...
        (done) => {
          if (!isDemoExists) {
            ...
            return gulp.src('**/*', {cwd: demoSrc, base: demoSrc}).pipe(gulp.dest(demoDist))
          }
    ...
    gulp.task(`${id}-watch-json`, () => gulp.watch(
      ...
      gulp.series(
        `${id}-component-check`,
        gulp.parallel(`${id}-component-wxml`, `${id}-component-wxss`, `${id}-component-js`, `${id}-component-json`)
      )
    ))
    ...
    gulp.task(`${id}-watch-demo`, () => {
      const demoSrc = config.demoSrc
      const demoDist = config.demoDist
      const watchCallback = (filePath) => gulp.src(filePath, {cwd: demoSrc, base: demoSrc}).pipe(gulp.dest(demoDist))
